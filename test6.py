...
def main():
    ...
    common_times = pd.to_datetime(np.intersect1d(df_act['time_obj'], df_actv['Start']))# Filter both DataFrames to keep only the common time points
    df_act = df_act[df_act['time_obj'].isin(common_times)]
    df_actv = df_actv[df_actv['Start'].isin(common_times)]
    ...
    df_actv = df_actv[df_actv['duration'] > 0]
