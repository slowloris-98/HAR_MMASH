...
# Pre-load the icons into a dictionary of images:
icon_images = {}
for act_code, icon_path in activity_icons.items():
    icon_images[act_code] = mpimg.imread(icon_path)  # Load as an array
...
def main():
    ...
    for idx, row in df_actv.iterrows():
        ...
        # If we have an icon for this activity, place it
        if act_code in icon_images:
            # Convert array to an OffsetImage
            icon_arr = icon_images[act_code]
            imagebox = OffsetImage(icon_arr, zoom=0.4)  # Adjust zoom for size
            ab = AnnotationBbox(
                imagebox, 
                (row['Start'], max_hr + 5),  # (x, y) where to place the center of the icon
            ...
            # Optional fallback if no icon is found
            plt.text(row['Start'], max_hr + 5, f"Act {act_code}", rotation=90, fontsize=8)
